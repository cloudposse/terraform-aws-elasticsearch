...
| <a name="input_auto_tune_cron_schedule"></a> [auto\_tune\_cron\_schedule](#input\_auto\_tune\_cron\_schedule) | A cron expression specifying the recurrence pattern for an Auto-Tune maintenance schedule. | `string` | `null` | no |
| <a name="input_auto_tune_duration"></a> [auto\_tune\_duration](#input\_auto\_tune\_duration) | Autotune maintanance window duration time in hours. | `number` | `null` | no |
